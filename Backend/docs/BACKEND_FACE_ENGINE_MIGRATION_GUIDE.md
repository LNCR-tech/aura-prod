...
- the backend no longer gates login behind face verification; these routes are available for profile security operations and manual verification flows
...
8. Test one privileged account login and confirm the API returns a direct bearer session without a face-pending token or MFA challenge step.
