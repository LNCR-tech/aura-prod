...
The Aura demo seeder is a standalone data generation tool that populates a fresh Aura database with a complete, realistic, production-scale dataset. It is not a fixture loader or a factory helper — it is a full simulation of a multi-school academic environment, including students, governance officers, events, attendance records, sanctions, and compliance history.
...
- A full governance hierarchy (SSG → SG → ORG) with officers drawn from the student body
...
- **QA and testing** — test every feature against realistic data without manual setup
- **AI assistant stress-testing** — the assistant queries live school data; it needs real data to answer real questions
- **Demo environments** — spin up a convincing demo instance for stakeholders
- **Development** — work on reports, dashboards, and analytics against data that actually exercises edge cases
...
Within the deterministic framework, the seeder deliberately introduces variance. Student enrollment is not uniform across programs — some programs get more students than others. Events are not all completed — some are cancelled, some are ongoing, some are upcoming. Absence rates are not fixed — they vary per student per event. Sanctions are not all pending — some are resolved, some are not.
...
The seeder is configured to generate large datasets by default (1,000–2,000 students per school, 30–100 events per school). This is intentional. Small datasets hide performance problems and produce misleading analytics. The seeder is designed to stress the system, not just populate it.
...
- It is not a bootstrap tool. Run `bootstrap.py` before the seeder — it seeds the global event types and base roles that the seeder depends on.
